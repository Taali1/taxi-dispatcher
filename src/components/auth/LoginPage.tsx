...
const LoginPage: React.FC = () => {
  ...
  const getCurrentRoleName = () => {
    return roles.find(role => role.id === selectedRole)?.name || 'Dyspozytor';
  };

  useEffect(() => {
    const targetText = getCurrentRoleName();
    setDisplayedTitle('');
    setIsTyping(true);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <select
                ...
                className="w-full px-4 py-3 bg-white dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-lg text-black dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
              >
                {roles.map((role) => (
